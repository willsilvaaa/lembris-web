document.addEventListener("DOMContentLoaded", () => {
  ...
  async function deletarAnotacaoAPI(id, titulo) {
    ...
    try {
      const response = await fetch(`${ANOTACOES_API_URL}${id}/`, {
        method: "DELETE",
      });
  ...
  async function carregarAnotacoes() {
    ...
    try {
      const response = await fetch(ANOTACOES_API_URL);
  ...
  if (formCriarFlashcard) {
    formCriarFlashcard.addEventListener("submit", async (event) => {
      ...
      try {
        ...
        // Cria o Conjunto 
        const conjuntoResponse = await fetch(CONJUNTOS_API_URL, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        // Cria o primeiro Flashcard associado ao conjunto
        const flashcardResponse = await fetch(FLASHCARDS_API_URL, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            conjunto: conjuntoId,
  ...
  async function loadConjuntoCards() {
    ...
    try {
      const response = await fetch(CONJUNTOS_API_URL);
      if (!response.ok) {
  ...
  function initializeFlashcardLogic() {
    ...
    favoriteIcons.forEach((icon) => {
      icon.addEventListener("click", async (event) => {
        ...
        try {
          const response = await fetch(`${CONJUNTOS_API_URL}${cardId}/`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
